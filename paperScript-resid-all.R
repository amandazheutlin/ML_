...
resid.input <- cbind(targets,cnp.df[c(snps,"age","gender","PC1","PC2","PC3","PC4","PC5")])
resid       <- lapply(pred.tar, function(x) {
  lm(eval(substitute(i ~ age + gender + PC1 + PC2 + PC3 + PC4 + PC5, 
                     list(i = as.name(x)))), data = resid.input)
})

resid_vals        <- lapply(resid, function(x) residuals(x)) %>% as.data.frame()
names(resid_vals) <- pred.tar
...
# create residual variables for all predictors and outcomes
resid_swe.input <- cbind(rep.predictors,rep.outcomes.s,raw_rep[,c("age","gender","PC1","PC2","PC3","PC4","PC5")]) %>% as.data.frame
resid_swe <- lapply(pred.tar, function(x) {
  lm(eval(substitute(i ~ age + gender + PC1 + PC2 + PC3 + PC4 + PC5, 
                     list(i = as.name(x)))), data = resid_swe.input)
})

resid_swe_vals        <- lapply(resid_swe, function(x) residuals(x)) %>% as.data.frame()
names(resid_swe_vals) <- pred.tar
...
write.table(outstats, "revision2/covar-resid-targets.txt", col.names=T, row.names=F, sep="\t", quote=F)

save.image("revision2/covar-resid.RData")
